...
def _streaming_confusion_matrix_at_thresholds(predictions,
  ...
  predictions_2d = array_ops.reshape(
      math_ops.cast(predictions, dtype=dtypes.float32), [-1, 1])
...
def _compute_dynamic_auc(labels, predictions, curve='ROC', weights=None):
  ...
  labels, predictions, weights = metrics_impl._remove_squeezable_dimensions(  # pylint: disable=protected-access
      labels, predictions, weights)
...
def precision_recall_at_equal_thresholds(labels,
  ...
  with variable_scope.variable_scope(name,
    ...
    # so we can use tf.compat.v1.scatter_add() to update the buckets in one
    # pass.
    #
    ...
    fn_convert = lambda tensor: math_ops.cast(tensor, out_dtype)
    result = PrecisionRecallData(
        tp=fn_convert(tp),
        fp=fn_convert(fp),
        tn=fn_convert(tn),
        fn=fn_convert(fn),
        precision=fn_convert(precision),
        recall=fn_convert(recall),
        thresholds=fn_convert(math_ops.lin_space(0.0, 1.0, num_thresholds)))
